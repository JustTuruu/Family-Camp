...
const ACCOMMODATION_ICONS: Record<string, React.ReactNode> = {
  ger: (
    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
      <path strokeLinecap="round" strokeLinejoin="round" d="M3 11l9-7 9 7M5 11v9h14v-9M10 20v-5h4v5" />
    </svg>
  ),
  shovgor: (
    <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} aria-hidden="true">
      ...
      <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
          {ACCOMMODATIONS.map((plan, index) => {
            const tiers = [
              { label: "Өдрийн байрлал", price: `₮${plan.pricing.perDay.toLocaleString()}`, highlight: false, note: "өдөр" },
              { label: "1 шөнө", price: `₮${plan.pricing.perNight.toLocaleString()}`, highlight: true, note: "шөнө бүр" },
              { label: "2+ шөнө", price: `₮${plan.pricing.perNight2Plus.toLocaleString()}`, highlight: false, note: "шөнө бүр" },
            ];
            ...
            return (
              <motion.div
                ...
                className={`relative bg-white border rounded-3xl shadow-lg shadow-bark/5 overflow-hidden ${
                  isPopular ? "border-ember/40 ring-1 ring-ember/30" : "border-sand"
                }`}
              >
                {isPopular && (
                  <div className="absolute top-5 right-5 bg-ember text-cream text-[0.6rem] uppercase tracking-widest px-3 py-1 rounded-full font-body shadow-md">
                    Хамгийн алдартай
                  </div>
                )}
                ...
                <div className="px-7 pt-7 pb-5">
                  <div className="flex items-center gap-3 mb-1">
                    <span className="w-11 h-11 rounded-xl bg-forest/10 text-forest flex items-center justify-center" aria-hidden="true">
                ...
                <div className="px-7 pb-8 border-t border-sand/60 pt-6">
                  ...
                  <ul className="space-y-2">
                    {(INCLUDES[plan.id] ?? []).map((item) => (
                      ...
                      >
                        <span className="w-4 h-4 rounded-full bg-forest/15 flex items-center justify-center flex-shrink-0" aria-hidden="true">
                          <svg
        ...
        >
          ...
          <p className="text-bark/45 text-xs mt-6 font-body">
            Утсаар:{" "}
            <a href={`tel:${SITE.phone.replace(/\s/g, "")}`} className="text-bark/70 hover:text-ember transition-colors">
              {SITE.phone}
            </a>
          </p>
        </motion.div>
